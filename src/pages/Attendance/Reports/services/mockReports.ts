...
export const getDailyReport = async (
  ...
  return Array.from({ length: 25 }).map((_, i) => {
    ...
    return {
      ...
      entryTime: isAbsent ? "--" : `0${8 + (isLate ? 1 : 0)}:${Math.floor(Math.random() * 59).toString().padStart(2, '0')} AM`,
      lastSeenAt: isAbsent ? "--" : `0${9 + Math.floor(Math.random() * 8)}:${Math.floor(Math.random() * 59).toString().padStart(2, '0')} AM`,
      currentStatus: isAbsent || !isMissingExit ? "Out" : "In",
      exitTime: isAbsent || isMissingExit ? "--" : `0${5 + Math.floor(Math.random() * 2)}:${Math.floor(Math.random() * 59).toString().padStart(2, '0')} PM`,
